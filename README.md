...
| 물류비 | **배송단가 시스템 연동** — 기존 시스템이 산정한 박스당 배송단가를 그대로 사용 (직접입력으로 덮어쓰기 가능) |
...
- 경로 설정: "물류비 (배송단가 연동)" 화면에서 폴더 경로 변경 가능 (기본 `C:\Users\HanEx\Desktop\delivery_pricing`)
- 우선순위: 배송비 직접입력 > 연동
...
- **종합 견적** (`/`) — 고객사 연동 또는 물동 직접 입력 → 3대 비용 산출 → 최종 견적단가 + 민감도 시나리오 + 견적 저장
- **작업비 마스터** (`/masters/work-cost`) — 공정별 생산성·시급, 간접배부율
- **물류비 (배송단가 연동)** (`/masters/delivery-link`) — 배송단가 시스템 고객사별 단가 조회·캐시 새로고침·경로 설정
- **보관비 마스터** (`/masters/storage`) — 센터별 임차료·관리비·유효 CAPA·목표가동률

기본 등록된 공정 생산성·시급은 개략 기준값이므로 실제 원가로 수정 후 사용하세요.
