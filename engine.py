...
PROCESS_DEFS = [
    ...
    ('입고 검수',      '입고', 'BOX', 150, '일용',   'A6h — 옵션 공정: 검수 입고 화주 견적에서만 체크해 사용'),
...
def compute(s, params, processes, region_rates, delivery, stages=None, customs=None):
    ...
    stages_out = [
        {'key': 'inbound', 'name': '입고 작업비', 'icon': 'bi-box-arrow-in-down',
         ...
         'cost_min': round(inbound_plt_rate), 'cost_max': round(inbound_plt_rate),
         'cpb': round(inbound_cpb, 2),
         'volume_label': f"월 입고 {_w(in_plt_month)}PLT",
         'monthly_cost_min': round(inbound_plt_rate * in_plt_month),
         'monthly_cost_max': round(inbound_plt_rate * in_plt_month),
        ...
        {'key': 'outbound', 'name': '출고 작업비', 'icon': 'bi-box-arrow-up',
         ...
         'cost_min': round(outbound_cpb, 1), 'cost_max': round(outbound_cpb, 1),
         'cpb': round(outbound_cpb, 2),
         'volume_label': f"월 출고 {_w(monthly_box)}BOX",
         'monthly_cost_min': round(outbound_cpb * monthly_box),
         'monthly_cost_max': round(outbound_cpb * monthly_box),
        ...
        {'key': 'delivery', 'name': '배송비', 'icon': 'bi-truck',
         ...
         'cost_min': round(dv_cost_min, 1), 'cost_max': round(dv_cost_max, 1),
         'cpb': round((dv_cost_min + dv_cost_max) / 2, 2),
         'volume_label': f"월 출고 {_w(monthly_box)}BOX",
         'monthly_cost_min': round(dv_cost_min * monthly_box),
         'monthly_cost_max': round(dv_cost_max * monthly_box),
